        ...
        <nav className={`navbar navbar-expand-sm navbar-${handleDarkMode} headroom headroom--not-bottom headroom--pinned headroom--top`}>
            <div className="container">
                ...
                <Div.Box className="collapse navbar-collapse" id="navbarNav">
                    <Div.Box component="ul" className="navbar-nav mx-auto">
                        {Navigate.map((link, index) => (
                            <li className="nav-item" key={index}>
                                <ReactDOM.Link to={link.to} onClick={() => setActive(link.active)} className={`nav-link ${link.active === active && !link.type && 'active'}`}>
